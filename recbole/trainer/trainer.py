...
class Trainer(AbstractTrainer):
    ...
    def fit_dis(self, train_data, valid_data, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score = self.dis_evaluate(valid_data, load_best_model=False, show_progress=show_progress)
    ...
    @torch.no_grad()
    def dis_evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        if not eval_data:
            return
        ...
        for batch_idx, batched_data in iter_data:
            if eval_data.dl_type == DataLoaderType.FULL:
                ...
                result += scores
        print("Result !!!!!!!!", result)
        #print(len(eval_data.uid_list))
        result = float(result) / len(eval_data.uid_list)
